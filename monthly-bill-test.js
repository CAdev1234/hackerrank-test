...
function monthlyCharge(month, subscription, users) {
  // your code here!
  const total = users.reduce((prev, currUser) => {
    const firstDay = firstDayOfMonth(new Date(month).toLocaleDateString())
    const lastDay = lastDayOfMonth(new Date(month).toLocaleDateString())
    if (currUser.activatedOn.getTime() >= firstDay.getTime() &&
        currUser.activatedOn.getTime() <= lastDay.getTime()) {
       return prev + subscription.monthlyPriceInCents;
    }    
  }, 0) || 0;
  return total;
}
